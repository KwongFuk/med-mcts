...
class Planner:
    ...
    def generate_plan(self, current_state, reflection_material, image_path):
        ...
        # 构造 Prompt
        prompt = (
            f"<image> Generate a step-by-step reasoning plan for the following query based on the current query state "
            ...
            f"Provide a detailed reasoning plan as a numbered list."
        )
        ...
        # 解码生成的推理计划
        plans = [self.processor.decode(seq, skip_special_tokens=True).strip() for seq in output]
